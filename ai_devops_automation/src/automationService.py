...
from ai_devops_automation.src.dvcSteps import outputFile, errorFile, path_to_output_file
...
def gitCommands():
    ...
    git_push = 'git push origin '+Constants.DEVELOP_BRANCH
...
def gitIgnore():
    git_basic=  [ 'echo *.json >> .gitignore',
                  'echo *.pyc >> .gitignore',
                  'echo *.txt >> .gitignore',
                  'git remote add origin '+Constants.GIT_REMOTE,
                  'git checkout -b '+Constants.DEVELOP_BRANCH
                 ]
...
def checkForUpdates():
    ...
    if (code_changes and data_changes):
        ...
        dvc_dataupdate = [
                          'dvc add '+ data_dir_path,
                          'dvc push']
        print("\n \033[93m Both Code and Data files are updated, pushing changes to dvc remote and github\033[0m")
        for cmd in dvc_dataupdate:
            p = subprocess.Popen(cmd, stdout=outputFile, stderr=subprocess.PIPE, universal_newlines=True,shell=True)
            print(Fore.GREEN +"\n"+ cmd)
        ...
        print("\n\033[93m Only Code is updated, pushing changes to github \033[0m")
        update_codeConffile(updated_code_dict)
        gitCommands()
        ...
        dvc_dataupdate = ['dvc add '+ data_dir_path,
                          'dvc push']
        print("\n \033[93mData files are updated, adding updated data to dvc cache and pushing files to remote server \033[0m")
        for cmd in dvc_dataupdate:
            p = subprocess.Popen(cmd, stdout=outputFile, stderr=subprocess.PIPE, universal_newlines=True,
                                 shell=True)
            print(Fore.GREEN +"\n"+ cmd)
            print(Style.RESET_ALL)
            p.communicate()
